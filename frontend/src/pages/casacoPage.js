...
const CasacoPage = () => {
  ...
  return (
    <>
      ...
      <main className="casacoPagecontainer">
        <section className="casacoPageSection">
          <h1>Casacos</h1>
          <div className="casacoPageGride">
            {produtosPaginaAtual.length > 0 ? (
              produtosPaginaAtual.map(produto => (
                <div className="casacoProduto" key={produto.id}>
                  <Link to={isAdmin ? `/admProdutoPage/${produto.id}` : `/produtoPage/${produto.id}`}>
                    <img src={produto.cover_image} alt={produto.productname} />
                  </Link>
                  <div className="casacoDetalhes">
                    <p className="nomeCasaco">{produto.productname}</p>
                    <p className="precoCasaco">R$ {produto.price}</p>
                  </div>
                </div>
              ))
            ) : (
