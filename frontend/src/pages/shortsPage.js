...
const ShortsPage = () => {
  ...
  const fetchProdutos = async () => {
    try {
      ...
      const novosProdutos = data.filter(produto => produto.category == 6);
  ...
  return (
    <>
      ...
      <main className="shortsPagecontainer">
        <section className="shortsPageSection">
          <h1>Shorts</h1>
          <div className="shortsPageGride">
            {produtosPaginaAtual.length > 0 ? (
              produtosPaginaAtual.map(produto => (
                <div className="shortsProduto" key={produto.id}>
                  <Link to={isAdmin ? `/admProdutoPage/${produto.id}` : `/produtoPage/${produto.id}`}>
                    <img src={produto.cover_image} alt={produto.productname} />
                  </Link>
                  <div className="shortsDetalhes">
                    <p className="nomeShorts">{produto.productname}</p>
                    <p className="precoShorts">R$ {produto.price}</p>
                  </div>
                </div>
              ))
            ) : (
