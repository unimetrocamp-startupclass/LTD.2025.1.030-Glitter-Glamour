...
const VestidoPage = () => {
  ...
  return (
    <>
      ...
      <main className="vestidoPagecontainer">
        <section className="vestidoPageSection">
          <h1>Vestidos</h1>
          <div className="vestidoPageGride">
            {produtosPaginaAtual.length > 0 ? (
              produtosPaginaAtual.map(produto => (
                <div className="vestidoProduto" key={produto.id}>
                  <Link to={isAdmin ? `/admProdutoPage/${produto.id}` : `/produtoPage/${produto.id}`}>
                    <img src={produto.cover_image} alt={produto.productname} />
                  </Link>
                  <div className="vestidoDetalhes">
                    <p className="nomeVestido">{produto.productname}</p>
                    <p className="precoVestido">R$ {produto.price}</p>
                  </div>
                </div>
              ))
            ) : (
